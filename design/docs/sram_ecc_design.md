...
| V1.1.0 | — | 2026-06-04 | 验证反馈(BUG SRAM-01) | rd_addr_o由时序逻辑改为组合逻辑(直接从haddr_i[9:2]取)，修复读路径一周期延迟导致返回前一地址数据 |
...
| ecc_err_o | Output | 1 | ECC 错误指示 |
| ecc_sec_o | Output | 1 | 单比特错误纠正指示 |
| ecc_ded_o | Output | 1 | 双比特错误检测指示 |

> **V1.1变更**: rd_addr_o改为组合逻辑(haddr_i[9:2]直通)，消除读路径一周期延迟。hready_o恒为1（零等待）。
